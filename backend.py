        ...
        if len(bahan_terpilih) < 2:
            ...
            try:
                ...
                if solusi.success:
                    ...
                    kar_list = [(g/100)*c for g, c in zip(hasil_gram, pd.to_numeric(bahan_terpilih["Karbohidrat (g)"]).values)]
                    
                    tabel_hasil = pd.DataFrame({
                        "Bahan Makanan": bahan_terpilih["Bahan Makanan"].values,
                        "Takaran Disarankan": [f"{g:,.1f} Gram" for g in hasil_gram],
                        "Kalori (Kkal)": [f"{k:,.1f}" for k in kal_list],
                        "Protein (g)": [f"{p:,.1f}" for p in pro_list],
                        "Lemak (g)": [f"{l:,.1f}" for l in lem_list],
                        "Karbo (g)": [f"{c:,.1f}" for c in kar_list],
                    ...
                    total_row = pd.DataFrame({
                        ...
                        "Karbo (g)": [f"{sum(kar_list):,.1f}"],
                        "Biaya Realisasi": [f"Rp {solusi.fun:,.0f}"]
                    })
                    ...
                    with kol_pie:
                        biaya_per_bahan = [(g/100)*h for g, h in zip(hasil_gram, array_harga) if g > 0.01]
                        label_bahan = [b for g, b in zip(hasil_gram, bahan_terpilih["Bahan Makanan"].values) if g > 0.01]
                        
                        fig_pie = go.Figure(data=[go.Pie(labels=label_bahan, values=biaya_per_bahan, hole=.4, 
                                                         marker_colors=['#EF8354', '#4F5D75', '#BFC0C0', '#2D3142'])])
                        fig_pie.update_layout(title="Distribusi Proporsi Biaya", paper_bgcolor='rgba(0,0,0,0)', font=dict(color='#FFFFFF'))
